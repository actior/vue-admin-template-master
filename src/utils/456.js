...
export function gridjs(
  ...
  const plassets = [1, 69, 4, 12, 18, 21, 25, 29, 34, 41, 42, 49, 53, 55, 59, 60, 65]; // 资产购买列表
  const gridData = [1, 69].includes(playerformLine.event)
    ? firstGrid(playerformLine, playerItem, roundNum)
    : credit.includes(playerformLine.event)
    ...
    : [7, 32, 67].includes(playerformLine.event)
    ? houseBuy(playerformLine, playerItem)
