        ...
        <el-card style="margin-bottom: 20px">
          ...
          >
            ...
            <el-form-item label="买卖数量" prop="assetsNum">
              <el-input-number v-model="formInline.assetsNum" :min="1" :max="30" label="描述文字" />
            </el-form-item>
            <el-form-item label="骰子/现金" prop="cashmoney">
              <el-input v-model.number="formInline.cashmoney"></el-input>
            </el-form-item>
            <el-form-item label="骰子是否" prop="dice">
              <el-select
                v-model="formInline.dice"
                placeholder="请选择骰子单双数"
                clearable
                @change="handleDice"
              >
      ...
      this.playerInfos.map((item, index) => {
        if (index === val) {
          if (item.creditRound !== 0) {
            ...
            item.credit = item.credit - item.credit / 10
          }
          ...
          let plaguereleaseSum = 0 // 瘟疫事件专属字段
          item.assetsArray.forEach((arr) => {
            if ([2, 4, 5, 6, 8, 11].includes(arr.type)) {
              plaguereleaseSum += arr.cashFlow
            }
          })
    ...
    handleImplemented(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.playerInfos.map((item, index) => {
            item = eventCounter(this.formInline, item, index, this.partmoney, this.roundNum)
            return item
          })
          window.sessionStorage.setItem('playerInfos', JSON.stringify(this.playerInfos))
