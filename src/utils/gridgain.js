export function gridjs(
  ...
  const gridData = credit.includes(playerformLine.event)
    ? secondGrid(playerformLine, playerItem, roundNum, playerIndex)
    : plassets.includes(playerformLine.event)
    ? fourthGrid(playerformLine, playerItem, copyAssets, roundNum, playerIndex)
    ...
    ? sixteenGrid(playerItem, playerformLine, playerIndex)
    : [27, 63, 70].includes(playerformLine.event)
    ? auction(playerformLine, playerItem, playerIndex)
    : [39, 48, 62].includes(playerformLine.event)
    ? taxado(playerformLine, playerItem, playerIndex)
    : [5, 13, 24, 28, 31, 36, 44, 51, 57, 64].includes(playerformLine.event)
    ? skillMoney(playerItem, playerformLine, copySkill, playerIndex)
    : false;
  return gridData;
}
// 银行信贷信用卡
